...
export function ProjectCard({ project, onUpdated, onDeleted }: ProjectCardProps) {
  ...
  return (
    <div className="group relative flex flex-col gap-3 rounded-lg border border-white/[0.06] bg-card p-4 transition-colors hover:border-white/[0.12]">
      {/* Color accent bar */}
      <div
      ...
      <div className="pl-2 flex items-start justify-between gap-2">
        <div className="flex items-center gap-2 min-w-0">
          {project.icon && (
            <span className="text-lg leading-none shrink-0">{project.icon}</span>
          )}
          {!project.icon && (
            <span
