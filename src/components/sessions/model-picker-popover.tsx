'use client';

export interface ModelOption {
  id: string;
  label: string;
  description: string;
}

// Claude Code CLI accepts shorthand names: opus, sonnet, haiku
// (same as ~/.claude/settings.json "model": "sonnet")
export const AVAILABLE_MODELS: ModelOption[] = [
  { id: 'opus',   label: 'Claude Opus',   description: 'Most powerful' },
  { id: 'sonnet', label: 'Claude Sonnet', description: 'Balanced · default' },
  { id: 'haiku',  label: 'Claude Haiku',  description: 'Fast & efficient' },
];

interface ModelPickerPopoverProps {
  onSelect: (modelId: string) => void;
  onClose: () => void;
}

export function ModelPickerPopover({ onSelect, onClose }: ModelPickerPopoverProps) {
  return (
    <div className="absolute bottom-full left-0 right-0 mb-1 z-50 rounded-lg border border-white/[0.10] bg-[oklch(0.10_0_0)] shadow-2xl overflow-hidden">
      <div className="px-3 py-1.5 border-b border-white/[0.06]">
        <span className="text-[10px] text-muted-foreground/50 uppercase tracking-wider">
          Switch model · click to select
        </span>
      </div>
      <ul className="py-1">
        {AVAILABLE_MODELS.map((model) => (
          <li
            key={model.id}
            className="flex items-center gap-3 px-3 py-2.5 cursor-pointer hover:bg-white/[0.06] transition-colors"
            onClick={() => {
              onSelect(model.id);
              onClose();
            }}
          >
            <div className="flex-1 min-w-0">
              <span className="text-xs font-medium text-foreground">{model.label}</span>
              <span className="ml-2 text-[10px] text-muted-foreground/50">{model.description}</span>
            </div>
          </li>
        ))}
      </ul>
      <div className="px-3 py-1.5 border-t border-white/[0.06]">
        <button
          type="button"
          onClick={onClose}
          className="text-[10px] text-muted-foreground/40 hover:text-muted-foreground/70 transition-colors"
        >
          Cancel
        </button>
      </div>
    </div>
  );
}
