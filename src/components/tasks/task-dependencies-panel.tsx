...
export function TaskDependenciesPanel({ taskId }: TaskDependenciesPanelProps) {
  ...
  const [dependents, setDependents] = useState<Dep[]>([]);
  ...
  useEffect(() => {
    ...
    const timeout = setTimeout(() => {
      searchTasks(taskId, dependencies, searchQuery);
    }, 300);
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="flex flex-col gap-2">
        ...
        {dependents.map((dep) => (
          <div key={dep.id} className="flex items-center gap-2 rounded border px-3 py-2">
            <span className="text-sm">{dep.title}</span>
            <Badge variant="outline" className="text-xs">
              {dep.status}
            </Badge>
        ...
        {dependents.length === 0 && (
          <p className="text-sm text-muted-foreground">Blocks no tasks</p>
        )}
      </div>
    </div>
  );
