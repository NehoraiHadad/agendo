      ...
      <div className="flex items-start gap-2 p-3">
        ...
        >
          ...
          <div className="mt-2 flex items-center gap-1.5 flex-wrap">
            ...
            {/* GitHub issue badge */}
            {task.inputContext?.githubIssueNumber && (
              <a
                href={
                  (task.inputContext.githubIssueUrl as string | undefined) ??
                  `https://github.com/${project?.githubRepo ?? ''}/issues/${task.inputContext.githubIssueNumber}`
                }
                target="_blank"
                rel="noopener noreferrer"
                onClick={(e) => e.stopPropagation()}
                className="inline-flex items-center gap-1 text-[10px] text-muted-foreground/55 hover:text-foreground/80 bg-white/[0.04] border border-white/[0.08] rounded-full px-2 py-0.5 font-medium transition-colors"
                title={`GitHub Issue #${task.inputContext.githubIssueNumber}`}
              >
