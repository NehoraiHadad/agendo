/**
 * Shared token analysis logic for the Token Optimizer tab.
 *
...
export function computeSuggestions({
  ...
  // CLAUDE.md size (target: <800 tokens)
  ...
  // MEMORY.md size (target: <600 tokens)
  ...
  if (memoryMd && memoryMd.tokens > 600) {
    suggestions.push({
      ...
      detail:
        'Remove content that duplicates CLAUDE.md. Keep only: learnings, corrections, habit tracking. Condense verbose operational history to current rule only.',
  ...
  // Behavioural tip: always relevant
  suggestions.push({
    level: 'info',
