/**
 * Per-provider binary introspection for model discovery.
 *
 * Each provider reader inspects the locally installed CLI tool to extract
 * the list of available models without calling any remote API.
 *
 * - Codex:  reads ~/.codex/models_cache.json
 * - Claude: pipes the binary through `strings | grep` to extract picker entries
 * - Gemini: reads the installed gemini-cli-core models.js source file
 */
...
function safePipe(
  ...
  return new Promise((resolve, reject) => {
    ...
    const proc1 = spawn(cmd1, args1, { stdio: ['ignore', 'pipe', 'ignore'] });
    const proc2 = spawn(cmd2, args2, { stdio: [proc1.stdout, 'pipe', 'ignore'] });
...
export async function readCodexModels(): Promise<ModelOption[]> {
  try {
 ...
 */
export async function readClaudeModels(): Promise<ModelOption[]> {
  try {
 ...
 */
export async function readGeminiModels(): Promise<ModelOption[]> {
  try {
